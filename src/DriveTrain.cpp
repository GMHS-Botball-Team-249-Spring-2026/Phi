...
void DriveTrain::DriveLineTracking(int ticks, int speed)
{
    ...
    while (abs(gmpc(this->AM[0])) < target_ticks) // isn't ideal as it only checks one motor, but we will deal
    {
        ...
        if ((ON_LINE_FL && ON_LINE_FR) || (!ON_LINE_FL && !ON_LINE_FR))
        {
...
void DriveTrain::DriveForwardToLine(int speed)
{
    DriveToLine(speed);
}

void DriveTrain::DriveBackwardToLine(int speed)
{
    DriveToLine(-speed);
}
...
{
    CAMPC();
    int target_ticks = abs(ticks); // FIX: was 'ticks', caused infinite hang on negative ticks
    ...
    {
        // FIX: was (speed * ...) which drove backward; negate so motors spin forward
...
void DriveTrain::RotateLeft(int ticks, int speed)
{
    Rotate(ticks, -speed);
}

void DriveTrain::RotateRight(int ticks, int speed)
{
    Rotate(ticks, speed);
}
